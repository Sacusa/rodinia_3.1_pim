/**
 * The GEMM implementation is taken from:
 * https://github.com/Cjkkkk/CUDA_gemm/tree/master
 */
...
{
    float *A, *B, *C;
    cudaMalloc(&A, ASIZE(float));
    cudaMalloc(&B, BSIZE(float));
    cudaMalloc(&C, CSIZE(float));
    ...
    const bool ENABLE_DOUBLE_BUFFER = false;
     ...
     MatrixMulCUDA6<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_Y,
         THREAD_SIZE_X, ENABLE_DOUBLE_BUFFER>
             <<< dimGrid, dimBlock, 0, stream >>> (A, B, C, M, K, N, 2.0, 2.0);
