...
/* chip parameters	*/
float t_chip_3D = 0.0005;
float chip_height_3D = 0.016; float chip_width_3D = 0.016; /* ambient temperature, assuming no package at all	*/
float amb_temp_3D = 80.0;
        ...
        for(z = 0; z < nz; z++)
            for(y = 0; y < ny; y++)
                for(x = 0; x < nx; x++)
                {
                    ...
                    tOut[c] = tIn[c]*cc + tIn[n]*cn + tIn[s]*cs + tIn[e]*ce + tIn[w]*cw + tIn[t]*ct + tIn[b]*cb + (dt/Cap) * pIn[c] + ct*amb_temp_3D;
                }
        float *temp = tIn;
        tIn = tOut;
        tOut = temp;
...
void usage_3D(int argc, char **argv)
{
...
int main_hotspot3D(int argc, char** argv)
{
    ...
    float dx = chip_height_3D/numRows;
    float dy = chip_width_3D/numCols;
    float dz = t_chip_3D/layers;

    float Cap = FACTOR_CHIP * SPEC_HEAT_SI * t_chip_3D * dx * dy;
    float Rx = dy / (2.0 * K_SI * t_chip_3D * dx);
    float Ry = dx / (2.0 * K_SI * t_chip_3D * dy);
    float Rz = dz / (K_SI * dx * dy);

    float max_slope = MAX_PD / (FACTOR_CHIP * t_chip_3D * SPEC_HEAT_SI);
