...
unsigned int num_threads_kmeans = num_threads_perdim*num_threads_perdim;	/* number of threads */
unsigned int num_blocks_kmeans = num_blocks_perdim*num_blocks_perdim;		/* number of blocks */
...
void allocateMemory(int npoints, int nfeatures, int nclusters, float **features, cudaStream_t stream)
{
	...
	num_blocks_perdim = sqrt((double) num_blocks_kmeans);
	while (num_blocks_perdim * num_blocks_perdim < num_blocks_kmeans)	// defeat truncation (should run once)
		num_blocks_perdim++;

	num_blocks_kmeans = num_blocks_perdim*num_blocks_perdim;
