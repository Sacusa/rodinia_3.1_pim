...
int main_leukocyte(int argc, char ** argv) {
	...
	MAT *image_chopped = get_frame_leukocyte(cell_file, 0, 0, 0);
	...
	long long GICOV_start_time = get_time_leukocyte();
	MAT *gicov = GICOV(grad_x, grad_y);
	long long GICOV_end_time = get_time_leukocyte();

	// Dilate the GICOV matrices
	long long dilate_start_time = get_time_leukocyte();
	MAT *img_dilated = dilate(gicov);
	long long dilate_end_time = get_time_leukocyte();
	...
	printf("            Total: %.5f seconds\n", ((float) (get_time_leukocyte() - program_start_time)) / (1000*1000));
	...
	long long tracking_start_time = get_time_leukocyte();
	int num_snaxels = 20;
	ellipsetrack(cell_file, QAX_CENTERS, QAY_CENTERS, k_count, radius, num_snaxels, num_frames);
	printf("           Total: %.5f seconds\n", ((float) (get_time_leukocyte() - tracking_start_time)) / (float) (1000*1000*num_frames));
