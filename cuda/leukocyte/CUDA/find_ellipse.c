...
MAT * get_frame_leukocyte(avi_t *cell_file, int frame_num, int cropped, int scaled) {
	int dummy;
	...
	if (cropped) {
		...
		image_chopped = chop_flip_image_leukocyte(image_buf, height, width, TOP, BOTTOM, 0, width - 1, scaled);
	} else {
		// Just flip the image
		image_chopped = chop_flip_image_leukocyte(image_buf, height, width, 0, height - 1, 0, width - 1, scaled);
	}
...
// If scaled == true, all values are scaled to the range [0.0, 1.0
MAT * chop_flip_image_leukocyte(unsigned char *image, int height, int width, int top, int bottom, int left, int right, int scaled) {
