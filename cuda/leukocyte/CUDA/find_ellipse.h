...
extern long long get_time_leukocyte();

extern MAT * get_frame_leukocyte(avi_t *cell_file, int frame_num, int cropped, int scaled);
extern MAT * chop_flip_image_leukocyte(unsigned char *image, int height, int width, int top, int bottom, int left, int right, int scaled);
