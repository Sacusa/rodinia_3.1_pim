...
void ellipsetrack(avi_t *video, double *xc0, double *yc0, int Nc, int R, int Np, int Nf) {
	...
	*/
	...
	for (frame_num = 1; frame_num <= Nf; frame_num++) {	 
		...
		MAT *I = get_frame_leukocyte(video, frame_num, 0, 1);
		int Ih = I->m;
		int Iw = I->n;
		...
		long long MGVF_start_time = get_time_leukocyte();
		MAT **IMGVF = MGVF(IE, 1, 1, Nc);
		MGVF_time += get_time_leukocyte() - MGVF_start_time;
