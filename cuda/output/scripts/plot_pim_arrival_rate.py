...
def add_plot_single_pim(pim):
    def add_bar(offset, policy):
        ...
        plt.bar([i+offset for i in x], arrival_latency[policy],
    ...
    plt.xticks(x, [app_labels[app] for app in applications] + ['GMean'],
            fontsize=25)
    plt.ylabel('PIM Arrival Rate (normalized)', fontsize=30)
    ...
    # save the image
    plt.savefig('../plots/' + pim + '_pim_arrival_rate.pdf',
            bbox_inches='tight')
...
def add_plot_all_pim():
    ...
    # save the image
    plt.savefig('../plots/all_pim_arrival_rate.pdf',
            bbox_inches='tight')

base_arrival_latency = {pim:[] for pim in pim_kernels}

for pim in pim_kernels:
    base_arrival_latency[pim] = [0 for c in range(num_channels)]
    ...
    for line in open('/u/sgupta45/PIM_apps/STREAM/output/pim_rf_size_' +
        ...
        elif 'AvgPimReqArrivalLatency' in line:
            tokens = line.split()
            assert(len(tokens) == 3)
            base_arrival_latency[pim][channel] = float(tokens[2])

    base_arrival_latency[pim] = amean(base_arrival_latency[pim])
...
for pim in pim_kernels:
    if pim == 'stream_triad':
        ...
        for policy in policies:
            avg_arrival_latency[policy].append(
                    avg_arrival_latency[policy][stream_add_index])
        continue
    ...
    for policy in policies:
        ...
        for app in applications:
            ...
            arrival_latency[policy][-1] = base_arrival_latency[pim] / \
                    amean(arrival_latency[policy][-1])

        avg = gmean(arrival_latency[policy])
        arrival_latency[policy].append(avg)
        avg_arrival_latency[policy].append(avg)
