...
labels = {
        ...
        'mem_first': 'MEM',
        'pim_first': 'PIM',
        'frfcfs_cap_0': 'FR-FCFS',
        'frfcfs_cap_32': 'FR-FCFS-Cap',
        'bliss_interval_10000_threshold_4': 'BLISS',
        'fr_rr_fcfs': 'FR-RR-FCFS',
        'gi': 'G&I',
        'f3fs_cap_256_slowdown_1': 'F3FS'
}
...
def stats_db_exists():
    return pathlib.Path(stats_db_filename).is_file()
    ...
    with open(stats_db_filename, "w") as stats_db_file:
        for vc in [1, 2]:
            for p, policy in enumerate(policies):
                policy_name = policy[p] if vc == 1 else policies_vc_2[p]

                stats_db[policy_name] = {}

                for pim in pim_kernels:
                    stats_db[policy_name][pim] = {}

                    for app in applications:
                        print(policy_name, pim, app)
                        stats_db[policy_name][pim][app] = {}
                        ...
                        outdir = "../output_vc1/" if vc == 1 else \
                                 "../output_vc2/"

                        for line in open(outdir + policy + '/' + app + '_' + \
                                         pim):
                        ...
                        stats_db[policy_name][pim][app]["mem_time"] = mem_time
                        stats_db[policy_name][pim][app]["pim_time"] = pim_time
                        stats_db[policy_name][pim][app]["dram_mem_arrival_latency"] = \
                                arrival_latency
                        stats_db[policy_name][pim][app]["num_mode_switches"] = \
                                avg_mode_switches
                        stats_db[policy_name][pim][app]["switch_latency"] = \
                                switch_latency
                        stats_db[policy_name][pim][app]["switch_conflicts"] = \
                                switch_conflicts
                        stats_db[policy_name][pim][app]["bwutil"] = bwutil
...
def get_base_pim_exec_time():
    ...
    for pim in pim_kernels:
        ...
        for line in open('/opt/PIM_apps/STREAM/output/pim_rf_size_' +
