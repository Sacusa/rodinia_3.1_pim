...
#define ITER 5 					    // iterate ITER* k log k times; ITER >= 1
...
{
#ifdef PROFILE
  double t1 = gettime_streamcluster();
#endif
  long i, j;
  Point temp;
...
#ifdef PROFILE
  double t2 = gettime_streamcluster();
  time_shuffle += t2-t1;
#endif
}
...
//  }
  for (i=0;i<iter;i++) {
      x = i%numfeasible;
      change += pgain(feasible[x], points, z, k, kmax, is_center, center_table,
...
//  }
  cost = pFL(points, feasible, numfeasible,
    z, &k, kmax, cost, ITER, 0.1, pid, barrier);
#ifdef ENABLE_THREADS
  pthread_barrier_wait(barrier);
#endif
